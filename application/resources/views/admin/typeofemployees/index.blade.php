...
@section('page_title', __('Quản lý Loại nhân viên'))
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
                    <h4 class="card-title m-0">{{ __('Quản lý Loại nhân viên') }}</h4>
                    <div class="form-search form-inline">
                        <form action="">
                            <input type="text" name="keyword" placeholder="Tìm kiếm Loại nhân viên"
                    ...
                    <a href="{{ route('admin.typeofemployees.add') }}" class="btn btn-secondary">Thêm Loại nhân viên</a>

                </div>
                <form action="{{ route('admin.typeofemployees.action') }}" method="post">
                    @csrf
                    <div class="card-body">
                        <div class="analytic">
                                    ...
                                    @if ($typeofemployees->total() > 0)
                                        ...
                                        @foreach ($typeofemployees as $k => $v)
                                            <tr>
                                                ...
                                                <th scope="row">{{ $t++ }}</th>
                                                <td>{{ $v->typeofemployeess_code }}</td>
                                                <td>{{ $v->typeofemployeess_name }}</td>
                                                <td>
                                                    <div class="form-group col-md-6">
                                                        @if ($v->typeofemployeess_status == 1)
                                                ...
                                                <td>
                                                    <a class="btn btn-primary"
                                                        href="{{ route('admin.typeofemployees.edit', $v->id) }}"
                                                        role="button">Sửa</a>
                                                    <a class="btn btn-danger"
                                                        href="{{ route('admin.typeofemployees.delete', $v->id) }}"
                                                        role="button">Xoá</a>
                                                </td>
                                            </tr>
                                        @endforeach
