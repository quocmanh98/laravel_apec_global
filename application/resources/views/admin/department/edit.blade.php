@extends('layouts.app')
@php
$roomCode = "MBP" . time();
$create_at = date("Y-m-d H:i:s");
@endphp
@section('page_title', __('Sửa phòng ban '))

@section('page')
    <form action="{{ route('admin.department.update',$department->id) }}" method="post">
        @csrf

        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('Sửa phòng ban') }}</h4>
                    </div>
                    <div class="card-body">
                        <div class="basic-form">

                            <div class="form-row">
                                <div class="form-group col-md-12">
                                    <label>@lang('Mã phòng ban')</label>
                                    <input type="text"
                                        class="form-control @error('departments_code')is-invalid @enderror"
                                        placeholder="@lang('Mã phòng ban')" value="{{ $department->departments_code }}" disabled>
                                </div>

                                <div class="form-group col-md-12">
                                    <label>@lang('Tên phòng ban')</label>
                                    <input name="departments_name" value="{{ $department->departments_name }}" type="text"
                                        class="form-control @error('departments_name')is-invalid @enderror"
                                        placeholder="@lang('Tên phòng ban')">
                                    @error('departments_name')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                                </div>

                                <div class="form-group col-md-6">
                                    <label>@lang('Slug')</label>
                                    <input name="departments_slug" value="{{ $department->departments_slug }}" type="text"
                                        class="form-control" disabled>
                                </div>

                                <div class="form-group col-md-6">
                                    <label>@lang('Trạng thái')</label>
                                    <select name="status" class="form-control">
                                        <option value="0" @if (old('status', $department->departments_status) == 0) selected @endif>Ẩn</option>
                                        <option value="1" @if (old('status', $department->departments_status) == 1) selected @endif>Hiện</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                                </div>

                                <div class="form-group col-md-12">
                                    <input type="submit" class="btn btn-success btn-lg" value='Sửa'>
                                </div>


                        </div>
                    </div>
                </div>
            </div>
        </div>
    </form>
@endsection
